                                           ...
                                           double alpha)
{
    ...
    if (r_is_objc_ptr(visual)) {
        bool resizeRole = role && strcmp(role, "resize") == 0;
        StripRect vr = resizeRole
            ? (StripRect){ frame.width - 48.0, frame.height - 48.0, 38.0, 38.0 }
            : (StripRect){ (frame.width - 116.0) / 2.0, 14.0, 116.0, 7.0 };
        ...
        r_msg2_main(visual, "setAutoresizingMask:", resizeRole ? (1 | 8) : (1 | 4), 0, 0, 0);
        stagestrip_set_background_white(visual, 1.0, resizeRole ? 0.30 : 0.36);
        uint64_t visualLayer = r_msg2_main(visual, "layer", 0, 0, 0, 0);
        if (r_is_objc_ptr(visualLayer)) {
            stagestrip_send_double(visualLayer, "setCornerRadius:", resizeRole ? 10.0 : 3.5);
                                                    ...
                                                    double side)
{
    ...
    // Stroke colour: white at 0.55 alpha. Stroke width 2.5.
    uint64_t UIColor = r_class("UIColor");
    ...
    stagestrip_send_double(shape, "setLineWidth:", 2.5);
    ...
    // Initial opacity: 0.6 (translucent but clearly visible). Control loop will
    // bump to 0.85 while the user is actively resizing, then snap back to 0
    // when the resize gesture ends (and restore to 0.6 on next begin).
    stagestrip_send_double(shape, "setOpacity:", 0.6);
                                                ...
                                                double h)
{
    ...
    S->cornerArcsVisible = true; // initial state matches the 0.6 opacity above
...
// Install an X close button at the top-right corner of a slot's window. When
                                                  ...
                                                  double w)
{
    ...
    // Tap chain: hide host view, then hide the window. Control loop teardown
    // detects the hidden state on its next tick (every 50ms) and frees the slot.
...
static void stagestrip_install_picker_app_tile(uint64_t panel,
                                               const char *bid,
...
{
    if (!r_is_objc_ptr(panel) || !bid || !*bid) return;
    ...
    if (r_is_objc_ptr(bidStr)) {
        ...
        stagestrip_add_invocation_action(tile,
            stagestrip_make_int_invocation(panel, "setTag:", kStripPickerCmdIconTap));
        r_msg2_main(bidStr, "release", 0, 0, 0, 0);
    }

    r_msg2_main(panel, "addSubview:", tile, 0, 0, 0);
}
                                                    ...
                                                    uint64_t pickerT0)
{
    ...
    printf("[STAGE] picker: library build pending count=%d (control loop will install)\n", n);
}
...
static bool stagestrip_control_loop_progress_library_build(void)
{
    ...
    if (c->startedAtMs == 0) {
        c->startedAtMs = stagestrip_now_ms();
                                                  ...
                                                  double sh)
{
    ...
    if (r_is_objc_ptr(cog)) {
        ...
        stagestrip_add_invocation_action(cog,
            stagestrip_make_bool_invocation(overlayWin, "setHidden:", true));
    ...
    for (int slot = 0; slot < 2; slot++) {
        ...
        if (initialBidForLabel[0]) {
            char shortName[64] = {0};
            stagestrip_bid_short_name(initialBidForLabel, shortName, sizeof(shortName));
            snprintf(chipText, sizeof(chipText), "%s: %s",
                     slotPrefix[slot],
                     shortName[0] ? shortName : initialBidForLabel);
        } else {
    ...
    int totalBids = stagestrip_collect_picker_bids(bids, names, 256);
                                                 ...
                                                 double sh)
{
    ...
    if (r_is_objc_ptr(hot)) {
        ...
        // Show the picker overlay window when the user swipes up OR taps the
        // dot. setHidden:NO works for both UIView and UIWindow.
        uint64_t showInv = stagestrip_make_bool_invocation(pickerOverlayWin, "setHidden:", false);
        uint64_t invokeSel = r_sel("invoke");
                                                     ...
                                                     double defaultY)
{
    ...
    // Host view fills the window.
    stagestrip_send_rect(hostView, "setFrame:", 0.0, 0.0, frame.width, frame.height);
    ...
    if (!r_is_objc_ptr(S->movePan) || !r_is_objc_ptr(S->resizePan)) {
        ...
        if (r_is_objc_ptr(S->moveHandle))
            r_msg2_main(win, "bringSubviewToFront:", S->moveHandle, 0, 0, 0);
        if (r_is_objc_ptr(S->resizeHandle))
            r_msg2_main(win, "bringSubviewToFront:", S->resizeHandle, 0, 0, 0);
    }
    // Per-slot X close button (top-right of the window). The tap chain hides
    // the window; the control loop tears down the slot on the next tick.
...
static int stagestrip_poll_picker_command(void)
{
    ...
    switch (cmd) {
        ...
        case kStripPickerCmdSwap: {
            ...
            char shortTop[64] = {0};
            char shortBottom[64] = {0};
            stagestrip_bid_short_name(bottom, shortTop, sizeof(shortTop));
            stagestrip_bid_short_name(top,    shortBottom, sizeof(shortBottom));
            uint64_t topChipStr = r_nsstr_retained(shortTop[0] ? shortTop : "—");
            uint64_t botChipStr = r_nsstr_retained(shortBottom[0] ? shortBottom : "—");
        ...
        case kStripPickerCmdStrip: {
            ...
            stagestrip_set_frame_thread(hostView, (StripRect){ 0.0, 0.0, target.width, target.height });
            stagestrip_relayout_stage_host_thread(hostView, target.width, target.height);
        ...
        case kStripPickerCmdIconTap: {
            ...
            char shortName[64] = {0};
            stagestrip_bid_short_name(pendingBid, shortName, sizeof(shortName));
            char chipText[96] = {0};
            snprintf(chipText, sizeof(chipText), "%s: %s",
                     slot == 0 ? "Top" : "Bottom",
                     shortName[0] ? shortName : pendingBid);
            ...
            char botAfter[128] = {0};
            if (r_is_objc_ptr(gStripPickerTopLabel))
                stagestrip_read_picker_label(gStripPickerTopLabel, topAfter, sizeof(topAfter));
            if (r_is_objc_ptr(gStripPickerBottomLabel))
                stagestrip_read_picker_label(gStripPickerBottomLabel, botAfter, sizeof(botAfter));
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        while (!gStripControlLoopStop) {
            ...
            for (int s = 0; s < kStripMaxFloatSlots; s++) {
                ...
                uint64_t winHidden = r_msg(win, r_sel("isHidden"), 0, 0, 0, 0);
                if (winHidden) {
                    moveActive[s] = false;
                    resizeActive[s] = false;
                    ...
                    if (!S->cornerArcsVisible) {
                        for (int c = 0; c < kStripCornerCount; c++) {
                            if (r_is_objc_ptr(S->cornerArcs[c]))
                                stagestrip_send_double(S->cornerArcs[c], "setOpacity:", 0.6);
                        }
                ...
                if (resizeActive[s] && activeCorner[s] >= 0 &&
                    ...
                    if (r_is_objc_ptr(pan) &&
                        ...
                        if (changed) {
                            ...
                            stagestrip_set_frame_thread(hostView,
                                (StripRect){ 0.0, 0.0, next.width, next.height });
                ...
                // Resize-end: clear active corner, hide the arcs so they're
                // invisible until the user starts another resize.
                ...
                // If arcs are hidden and the user is touching a corner (any
                // corner pan state in possible=0 with a touch, or any state >0),
                // we just bring them back when state == 1 (began) — that's
                // already handled in the resize-begin branch above.
